...
namespace VehicleCollisions
{
    ...
    public class VehicleCollisions : Callout
    {
        ...
        {
            //scene = new SceneFactory(random.Next(1, 11)).GetScene();
            scene = new SceneFactory(6).GetScene();
        ...
        public async void SpawnOnSceneCopCars()
        {
            policeCars = new Vehicle[scene.PoliceCars.Length];
            for (int i = 0; i < scene.PoliceCars.Length; i++)
            {
        ...
        public async void SpawnOnScenePedCops()
        {
            policePeds = new Ped[scene.PolicePeds.Length];
            for (int i = 0; i < scene.PolicePeds.Length; i++)
            {
        ...
        public async void SpawnObjects()
        {
            ...
            for (int i = 0; i < scene.ObjectModels.Length; i++)
            {
                var spawnedObject = scene.ObjectModels[i];
                
                // Request the model
                RequestModel((uint)spawnedObject.ModelHash);
            
                // Wait untill model has loaded
                while (!HasModelLoaded((uint)spawnedObject.ModelHash))
                {
                    await BaseScript.Delay(1000);
                }

                // Create the object
                spawnedObjects[i] = CreateObjectNoOffset((uint)spawnedObject.ModelHash,  spawnedObject.Location.X, spawnedObject.Location.Y,  spawnedObject.Location.Z, true, false, true);
        ...
        public async void SpawnCrashedCars()
        {
            crashedCars = new Vehicle[scene.CrashedCars.Length];
            for (int i = 0; i < scene.CrashedCars.Length; i++)
            {
                ...
                {
                    SetEntityRotation(crashedCars[i].Handle, crashedCar.Rotation[0], crashedCar.Rotation[1], crashedCar.Rotation[2], 1, true);
                }
                ...
                if (crashedCar.ShouldRandomBeDamaged && Utilities.RandomBool())
                {
                    ...
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, -0.84f, 2.21f, 0.22f, damage, 50.0f, true);
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, 0.84f, 2.21f, 0.22f, damage, 50.0f, true);
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, -0.84f, -2.21f, 0.22f, damage, 50.0f, true);
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, 0.84f, -2.21f, 0.22f, damage, 50.0f, true);
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, -1.0f, 0.0f, 0.22f, damage, 50.0f, true);
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, 1.0f, 0.0f, 0.22f, damage, 50.0f, true);
                    if(Utilities.RandomBool()) SetVehicleDamage(crashedCarHandle, 0.0f, 0.0f, 1.25f, damage, 50.0f, true);
                ...
                {
                    foreach (VehiclePed vehiclePed in crashedCar.Peds)
                    {
                        if (vehiclePed.ShouldRandomSpawn && Utilities.RandomBool())
                        {
                            continue;
                        }
                ...
                {
                     var trailer = await SpawnVehicle(crashedCar.AttachedTrailer.Model, crashedCar.AttachedTrailer.Location, crashedCar.AttachedTrailer.Heading);
                     
                     AttachVehicleToTrailer(crashedCars[i].Handle, trailer.Handle, 50f);

                     if (crashedCar.AttachedTrailer.VehicleOnTrailer != null)
                     {
                         var vehicleOnTrailer = await SpawnVehicle(crashedCar.AttachedTrailer.VehicleOnTrailer.Model, crashedCar.AttachedTrailer.VehicleOnTrailer.Location, crashedCar.AttachedTrailer.VehicleOnTrailer.Heading);
        ...
        public async void SpawnCivilians()
        {
            civilianPeds = new Ped[scene.CivilianPeds.Length];
            for (int i = 0; i < scene.CivilianPeds.Length; i++)
            {
        ...
        {
           foreach(string notification in scene.Notifications())
           {
        ...
        public override void OnCancelBefore()
        {
            try
            {
                if (crashedCars.Length > 0)
                {
                    ...
                    foreach (Vehicle spawnedCar in crashedCars)
                    {
                        if (spawnedCar == null) continue;
                        
                        spawnedCar.AttachedBlip?.Delete();
                        
                        // Remove ped blip
                        foreach (Ped passenger in spawnedCar.Occupants)
                        {
            ...
            {
                Debug.WriteLine($"Error in despawning crashed car blips: {e}");
            }
            ...
            try
            {
                if (spawnedObjects.Length > 0)
                {
                    ...
                    foreach (int spawnedObject in spawnedObjects)
                    {
            ...
            {
                Debug.WriteLine($"Error in despawning objects: {e}");
            }
            ...
            try
            {
                if (policeCars.Length > 0)
                {
                    ...
                    foreach (Vehicle policeCar in policeCars)
                    {
                        policeCar.Delete();
                    }
                }
            ...
            {
                Debug.WriteLine($"Error in despawning police cars: {e}");
            }
            ...
            try
            {
                ...
                {
                    // Remove the on-scene cop cars (if any)
                    foreach (Ped policePed in policePeds)
                    {
                        policePed.Delete();
                    }
                }
            ...
            {
                Debug.WriteLine($"Error in despawning police peds: {e}");
            }
            ...
            {
                Debug.WriteLine($"Error in finishing callout: {e}");
            }
