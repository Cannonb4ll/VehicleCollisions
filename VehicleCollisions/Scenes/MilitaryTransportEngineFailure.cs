...
namespace VehicleCollisions.Scenes
{
    internal class MilitaryTransportEngineFailure :IScene
    {
        ...
        private bool CarFixed = false;
        private int CarFixingTime = rnd.Next(15, 60) * 1000;
        ...
        {
            new CrashedVehicle(new Vector3(RandomCoordinates.X, RandomCoordinates.Y, RandomCoordinates.Z), AccidentHeadings[AccidentIndex], VehicleHash.Barracks2)
                ...
                {
                    new VehiclePed(new Vector3(RandomCoordinates.X, RandomCoordinates.Y, RandomCoordinates.Z), AccidentHeadings[AccidentIndex], PedUtilities.GetRandomMilitary(), VehicleSeat.Driver )
                        .ShouldHaveBlip(false), 
                })
                .AttachTrailer(
                    new CrashedVehicleTrailer(new Vector3(RandomCoordinates.X -5, RandomCoordinates.Y-5, RandomCoordinates.Z), AccidentHeadings[AccidentIndex], VehicleHash.TRFlat)
                        .SetVehicleOnTrailer(new CrashedVehicleTrailerVehicle(new Vector3(RandomCoordinates.X -5, RandomCoordinates.Y-5, RandomCoordinates.Z), 248f, TrailerVehicles[rnd.Next(0, TrailerVehicles.Length)]))
                )
        };
        
        public CivilianPed[] CivilianPeds => new CivilianPed[]
        {
            new CivilianPed(new Vector3(RandomCoordinates.X + 2f, RandomCoordinates.Y - 2f, RandomCoordinates.Z - 0.5f), 345.26f, PedUtilities.GetRandomMilitary())
        ...
        {
            this.SpawnedCivilianPeds = civilianPeds;
            this.SpawnedCrashedCars = crashedCars;
            
            CalloutStarted = Game.GameTime;
            
            ShowSubtitle("[Military officer] I am trying to fix this truck, please guard us while I fix this piece of junk", 12500);
        ...
        public void Finish()
        {
            ...
            if(!SpawnedCivilianPeds[0].IsInVehicle())
            {
                SpawnedCivilianPeds[0].Task.EnterVehicle(SpawnedCrashedCars[0]);
            }
        }

        public async Task RunAdditionalTasks()
        {
            if((Game.GameTime - CalloutStarted) < CarFixingTime)
            {
                return;
            }
        ...
        public async void TheTruckIsFixedAndCanDriveAway()
        {
            ...
            SpawnedCrashedCars[0].Driver.Task.DriveTo(SpawnedCrashedCars[0], new Vector3(-2333.18f, 3406.62f, 30.21f), 25f, 100f);
